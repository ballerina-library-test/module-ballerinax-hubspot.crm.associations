...
6. Click the Scopes tab and select the required scopes for CRM associations (such as `crm.objects.contacts.read`, `crm.objects.companies.read`, and other relevant CRM scopes).

7. Click Create app in the top right corner.

8. Review the info about your app's access token, then click Continue creating.

9. Copy the access token from the confirmation screen.
...
public function main() returns error? {
    ...
    crmAssociations:BatchResponseLabelsBetweenObjectPair response = check crmAssociationsClient->/associations/contact/company/batch/create.post(batchRequest);
}
```
