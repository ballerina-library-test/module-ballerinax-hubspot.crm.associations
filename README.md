...
[HubSpot](https://www.hubspot.com/) is a comprehensive customer relationship management (CRM) platform that provides tools for marketing, sales, customer service, and content management to help businesses grow and manage customer relationships effectively.
...
2. Ensure you have access to HubSpot's API features. While basic API access is available on free accounts, some advanced features may require paid plans (Starter, Professional, or Enterprise).
...
4. Click Create a private app and provide a name and description for your application.

5. Navigate to the Scopes tab and select the necessary scopes for CRM associations (typically including CRM objects read/write permissions).
...
public function main() returns error? {
    hscrmassoc:BatchInputPublicAssociationMultiPost batchRequest = {
        inputs: [
            {
                'from: {
                    id: "12345"
    ...
    hscrmassoc:BatchResponseLabelsBetweenObjectPair response = check hscrmassocClient->/associations/contacts/companies/batch/create.post(batchRequest);
}
```
