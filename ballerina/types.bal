...
# Response object containing report creation details and user information
public type ReportCreationResponse record {
    # Email of the user
    string userEmail;
    # ID of the user
    int:Signed32 userId;
    # DateTime object with timezone and date-only flag support
    DateTime enqueueTime;
};

# Association response containing source object and multiple labeled targets
...
public type BatchInputPublicDefaultAssociationMultiPost record {
    # Array of default association creation requests
...
public type BatchInputPublicAssociationMultiArchive record {
    # Array of association archive requests
...
# Detailed error information with context and remediation guidance
...
public type PublicAssociationMultiPost record {
    # Array of association type specifications to create
...
# Batch processing response for labels between object pairs operations
...
# Schema for archiving multiple associations from one source to multiple targets
public type PublicAssociationMultiArchive record {
    # Contains the Id of a Public Object
    PublicObjectId 'from;
    # Array of target object identifiers to archive associations with
    PublicObjectId[] to;
};
...
# Default association definition with specification and object identifiers
...
# Response collection containing multiple associated objects with labels
public type CollectionResponseMultiAssociatedObjectWithLabel record {
    # Pagination information for navigating through result sets
    Paging paging?;
    # Array of associated objects with label information
    MultiAssociatedObjectWithLabel[] results;
};

# Batch input container for multiple association creation operations
public type BatchInputPublicAssociationMultiPost record {
    # Array of association objects to process in the batch operation
    PublicAssociationMultiPost[] inputs;
};

# Default association definition for multiple object creation operations
...
public type BatchResponsePublicDefaultAssociation record {
    ...
    # An object containing relevant links related to the batch request
    record {|string...;|} links?;
    # Array of successfully processed default association results
...
# Request payload for batch fetching associations from multiple objects
public type BatchInputPublicFetchAssociationsBatchRequest record {
    # Array of individual association fetch requests to process in the batch
    PublicFetchAssociationsBatchRequest[] inputs;
};

# Response containing batch processing status and association results with labels
...
# Response containing batch processing status without specific result data
public type BatchResponseVoid record {
    ...
    # An object containing relevant links related to the batch request
    record {|string...;|} links?;
    # Array of empty results from completed batch operations
    anydata[] results;
    # Array of errors encountered during batch processing operations
    StandardError[] errors?;
    # The status of the batch processing request: "PENDING", "PROCESSING", "CANCELED", or "COMPLETE"
    "CANCELED"|"COMPLETE"|"PENDING"|"PROCESSING" status;
};

# Request to fetch associations for a specific object with pagination support
