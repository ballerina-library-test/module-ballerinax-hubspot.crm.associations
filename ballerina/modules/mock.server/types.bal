...
public type ReportCreationResponse record {
    ...
    # ID of the user
    int:Signed32 userId;
    # DateTime object with timezone and date-only indicator information
    DateTime enqueueTime;
};

# Association data with multiple labeled relationships between objects
public type PublicAssociationMultiWithLabel record {
    # Contains the Id of a Public Object
    PublicObjectId 'from;
    # Pagination information for navigating through result sets
    Paging paging?;
    # Array of target objects with their association labels and metadata
...
public type BatchInputPublicDefaultAssociationMultiPost record {
    # Array of default association creation requests to process in batch
...
# Batch processing response containing labels between object pairs
...
# Schema for archiving multiple associations from one object to many objects
...
# Response containing multiple associated objects with labels and pagination
...
# Batch request schema for creating multiple associations in a single operation
public type BatchInputPublicAssociationMultiPost record {
    # Array of association creation requests to process in the batch
    PublicAssociationMultiPost[] inputs;
};

# Schema for creating a default association between two specific objects
...
# Request object containing inputs for batch association fetch operations
public type BatchInputPublicFetchAssociationsBatchRequest record {
    # Array of association fetch requests to process in the batch operation
    PublicFetchAssociationsBatchRequest[] inputs;
};

# Response object for batch association operations with labeled results
...
# Response object for batch operations that return no specific result data
public type BatchResponseVoid record {
    ...
    # An object containing relevant links related to the batch request
    record {|string...;|} links?;
    # Array of void results from completed batch operations
    anydata[] results;
    # Array of errors encountered during void batch processing
    StandardError[] errors?;
    # The status of the batch processing request: "PENDING", "PROCESSING", "CANCELED", or "COMPLETE"
    "CANCELED"|"COMPLETE"|"PENDING"|"PROCESSING" status;
};

# Request object for fetching associations of a specific object
